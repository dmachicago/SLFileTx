...
//        var_dump(isset($argv));
    ...
    if ( strlen($sid) == 0 ) {
        print "ERROR: 03f missing login information." . PHP_EOL;
...
//$DB->$dbSid = $sid;
    ...
    $ckadmin = isAdmin($userid, $pwhash, $conn);
    
    if ( $ckadmin == 0 ) {
        $aResult['ERROR: '] = "ADMIN REQUIRED";
        $str = json_encode($aResult);
        
        return $str;
    }
    
    if ( $debug == 1 ) print 'ckadmin = ' . $ckadmin . PHP_EOL;
    ...
    $sql = "select GroupName from Groups order by GroupName " . PHP_EOL;
    
    if ( $debug == 1 ) print 'getGroupsAdmin SQL: sql = ' . $sql . PHP_EOL;
