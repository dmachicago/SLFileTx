...
//        var_dump(isset($argv));
    ...
    if ( defined('STDIN') ) {
        ...
        if ( $dmem_debug == 1 ) {
            if ( $dmem_debug == 1 ) {
                ...
                print "1a - Members Parameter $Members" . PHP_EOL;
            }
        }
        ...
        if ( isset($_POST['Members']) ) $sessionid = cleanString($_POST['$Members']);
        ...
        if ( $dmem_debug == 1 ) {
            ...
            print "1z - Members $Members" . PHP_EOL;
        }
    ...
    if ( strlen($sid) == 0 ) {
        print "ERROR: 03c missing login information." . PHP_EOL;
...
//$DB->$dbSid = $sid;
    ...
    if ( $rc == 0 ) {
        if ( $dmem_debug == 1 ) echo "*** LOGIN ERROR deleteMember ... Not Admin <$rc>" . PHP_EOL;
        LOGX("deleteMember LOGIN ERROR... Not Admin <$rc> / <$userid>");
    ...
    if ( strpos($Members, '|') !== false ) {
        $aGroups = explode("|", $Members);
        if ( $dmem_debug == 1 ) echo "- Members EXPLODED into array" . PHP_EOL;
        
        foreach ( $aGroups as $MemberID ) {
            $i += 1;
            if ( $dmem_debug == 1 ) echo "MemberID: '$MemberID' " . PHP_EOL;
            if ( $MemberID != '' ) {
                $rc = deleteMember($MemberID, $conn);
