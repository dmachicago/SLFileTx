...
//        var_dump(isset($argv));
    ...
    if ( strlen($sid) == 0 ) {
        print "ERROR: 03i missing login information." . PHP_EOL;
...
//$DB->$dbSid = $sid;
    ...
    if ( $rc == 0 ) {
        if ( $debug == 1 ) echo "*** LOGIN ERROR insertGroup ... Not Admin <$rc>" . PHP_EOL;
        LOGX("insertGroup LOGIN ERROR... Not Admin <$rc>");
    ...
    if ( strpos($Groups, '|') !== false ) {
        ...
        foreach ( $aGroups as $GroupName ) {
            ...
            if ( $GroupName != '' ) {
                $rc = insertGroup($GroupName, $conn);
                if ( $rc >= 0 ) {
                    $aResult['success'] = $GroupName;
                    $aResult['success'] = $rc;
                } else {
                    $aResult['ERROR'] = $rc;
                    $aResult['ERROR'] = "Failed to insert $GroupName";
                }
            }
        }
    } else {
        ...
        if ( $Groups != '' ) {
            $rc = insertGroup($Groups, $conn);
            $i = $rc;
            if ( $rc >= 0 ) {
                $aResult['success'] = $rc;
            } else {
                $aResult['ERROR'] = $rc;
                $aResult['ERROR'] = "Failed to insert $Groups";
            }
        }
    }
